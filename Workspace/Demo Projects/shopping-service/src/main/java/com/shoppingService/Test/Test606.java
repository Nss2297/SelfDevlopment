...
public class Test606 {
    ...
    private static void rotateImage(int noOfRows,int noOfCols,int rowIndex,int[][]matrix){
        int[] row=populateArray(noOfRows,matrix[rowIndex]);
//        int col=noOfCols-1
    }
        ...
        for(int row=0;row<noOfRows;row++){
            for(int col=0;col<noOfCols;col++){
                if(matrix[row][col]<-1000||matrix[row][col]>1000){
                    System.out.println(matrix[row][col]+" at row="+row+", col="+col+" is out of limit");
                    return false;
                }
            }
        }
    ...
    private static void rotate(int noOfRows,int noOfCols,int[][]matrix){
        if(noOfRows==noOfCols&&noOfRows<21&&limitCheck(noOfRows,noOfCols,matrix))rotateImage(noOfRows,noOfCols,0,matrix);
    }
