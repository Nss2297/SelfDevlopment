...
public class Test545 {
    ...
    private static void merge(int low, int high, int median, int[] aray) {
        int[] leftArray = populateArray(median - low + 1, low, aray);
        int[] rightArray = populateArray(high - median, median + 1, aray);
        int leftPnt = 0;
        int rightPnt = 0;
        int pnt = low;
        while (leftPnt < leftArray.length && rightPnt < rightArray.length) {
            if (leftArray[leftPnt] < rightArray[rightPnt]) {
                aray[pnt] = leftArray[leftPnt];
                ++leftPnt;
            } else {
                aray[pnt] = rightArray[rightPnt];
                ++rightPnt;
            }
            ++pnt;
        }
