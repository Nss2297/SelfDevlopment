...
public class Test484 {
    private static void fitRowIntoColumn(int[] array, int colIndex, int rowLength, int[][] matrix) {
        for (int row = 0; row < rowLength; row++) {
            matrix[row][colIndex] = array[row];
        }
    }
    ...
    private static void rotateImage(int rowLength, int colLength, int num, int[][] matrix) {
        if (limitCheck(matrix[num])) {
            ...
            fitRowIntoColumn(row, colIndex, rowLength, matrix);
        }
    }
