...
public class Test625 {
    ...
    private static int[] populateArray(int size, int[] array) {
    ...
    private static void rotateImage(int noOfRows, int noOfCols, int index, int[][] matrix) {
        int[] row = populateArray(noOfCols, matrix[index]);
        int col = noOfCols - 1 - index;
        if (index + 1 < noOfRows) rotateImage(noOfRows, noOfCols, index + 1, matrix);
        ...
        for (int row = 0; row < noOfRows; row++) {
            for (int col = 0; col < noOfCols; col++) {
                if (matrix[row][col] < -1000 || matrix[row][col] > 1000) {
                    System.out.println(matrix[row][col] + " at matrx[" + row + "][" + col + "] is out of limit.");
                    return false;
                }
            }
        }
    ...
    private static void rotate(int noOfRows, int noOfCols, int[][] matrix) {
        if (noOfRows == noOfCols && noOfRows < 2 && limitCheck(noOfRows, noOfCols, matrix))
