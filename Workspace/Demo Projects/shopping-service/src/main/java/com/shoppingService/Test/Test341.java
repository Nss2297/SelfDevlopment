...
public class Test341 {
    ...
    private static void merge(int low, int high, int median, int[] array) {
        ...
        int righPnt = 0;
        int pnt = low;
        while (leftPnt < leftArray.length && righPnt < rightArray.length) {
            if (leftArray[leftPnt] < rightArray[righPnt]) {
                array[pnt] = leftArray[leftPnt];
                ++leftPnt;
            } else {
                array[pnt] = rightArray[righPnt];
                ++righPnt;
            }
            ++pnt;
        }
        ...
        while (righPnt < rightArray.length) {
            array[pnt] = rightArray[righPnt];
            ++righPnt;
            ++pnt;
        }
