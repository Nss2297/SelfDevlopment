...
public class Test444 {
    ...
    private static void merge(int low, int high, int median, int[] array) {
        int[] lefArray = populateArray(median - low + 1, low, array);
        int[] rightArray = populateArray(high - median, median + 1, array);
        int leftPnt = 0;
        int rightPnt = 0;
        int pnt = low;
        while (leftPnt < lefArray.length && rightPnt < rightArray.length) {
            if (lefArray[leftPnt] < rightArray[rightPnt]) {
                array[pnt] = lefArray[leftPnt];
                ++leftPnt;
            } else {
