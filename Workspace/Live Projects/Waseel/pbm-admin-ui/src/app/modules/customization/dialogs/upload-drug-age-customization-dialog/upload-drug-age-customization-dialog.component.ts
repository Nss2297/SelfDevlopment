...
@Component({
    selector: 'app-upload-drug-age-customization-dialog',
    templateUrl: './upload-drug-age-customization-dialog.component.html',
    styleUrls: ['./upload-drug-age-customization-dialog.component.css']
})
export class UploadDrugAgeCustomizationDialogComponent {
    @Input('onClose')
    ...
    downloadSampleFile() {
        const sampleExcelUrl = 'assets/admin/Drug to Age Customization Sample File.xlsx';
        const payersampleExcelUrl = 'assets/payer/Drug to Age Customization Sample File.xlsx';
    ...
    uploadDrugAgeCustomizationData() {
        let uploadtitle = "";
        let uploadSubtitle = "";
        this.responseList = [];
        ...
        this.dialogService.showConfirmDialog(uploadtitle, uploadSubtitle, (data: any) => {
            ...
            if (data) {
                ...
                if (this.selectedFile) {
                    ...
                    this.dssCustomizationSVC.uploadDrugAgeCustomizationExcelDatafile(this.selectedFile, isOverrideParam).subscribe({
                        next: (data) => {
                            if (data.errorList) {
                                this.authService.showSystemLoader();
                            ...
                            if ((this.errorList.length || this.duplicateList.length) > 0) {
                                ...
                                this.isOverride = false;
                                this.fileSelected = false;
                                this.translate.get('customization.drugAgeUploadSuccessMsg').subscribe(text => uploadedMsg = text);
