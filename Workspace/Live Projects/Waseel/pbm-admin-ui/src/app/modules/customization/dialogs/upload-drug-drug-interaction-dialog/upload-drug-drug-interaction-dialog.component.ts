...
@Component({
    selector: 'app-upload-drug-drug-interaction-dialog',
    templateUrl: './upload-drug-drug-interaction-dialog.component.html',
    styleUrls: ['./upload-drug-drug-interaction-dialog.component.css']
})
export class UploadDrugDrugInteractionDialogComponent {
    ...
    downloadSampleFile() {
        const sampleExcelUrl = 'assets/admin/Drug to Drug Customization Sample File.xlsx';
        const payersampleExcelUrl = 'assets/payer/Drug to Drug Customization Sample File.xlsx';
        ...
        this.dialogService.showConfirmDialog(uploadtitle, uploadSubtitle, (data: any) => {
            ...
            if (data) {
                ...
                if (this.selectedFile) {
                    ...
                    this.dssCustomizationSVC.uploadDrugToDrugExcelDatafile(this.selectedFile, isOverrideParam).subscribe({
                        next: (data) => {
                            if (data.errorList) {
                                this.authService.showSystemLoader();
                            ...
                            if ((this.errorList.length || this.duplicateList.length) > 0) {
                                ...
                                this.isOverride = false;
                                this.fileSelected = false;
                                this.translate.get('customization.drugTodrugUploadSuccessMsg').subscribe(text => uploadedMsg = text);
