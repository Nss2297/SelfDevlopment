...
public class NewRequestService {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(NewRequestService.class);
	...
	public DssResponse manageNewDssRequest(DssRequest dssRequest, ContentCachingRequestWrapper requestWrapper,
		...
		if (tranLog != null) {
			/*
			 * Need to set session to avoid separate entry in transactionLog table if there
			 * is any failure occurs
			 */
			...
			if (dssResponse != null && dssResponse.getHttpStatusCode() == 200) {
				dssDataManipulationService.saveDssRequest(dssRequest, requestType, dssResponse.getResults());
			}
			if (dssResponse != null ) {
				LOGGER.info("Data saved in TransactionLog table for NEW Request. TransactionLogId is "
