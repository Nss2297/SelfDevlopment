...
public class DuplicateTherapySearchModel implements Specification<PCDuplicateTherapy> {
    ...
    @Override
    public Predicate toPredicate(Root<PCDuplicateTherapy> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        List<Predicate> predicates = new ArrayList<>();
        if (!StringUtils.isBlank(interactedServiceCode)) {
            predicates.add(criteriaBuilder.like(root.get("id").get("interactedServiceCode"), "%" + interactedServiceCode + "%"));
        }
