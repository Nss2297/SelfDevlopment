package com.waseel.pbm.payercustomizationservice.filter;
...
import com.waseel.pbm.payercustomizationservice.enums.URL;
...
public class RestApiLoggingFilter extends OncePerRequestFilter {
	...
	protected void doFilterWrapped(ContentCachingRequestWrapper request, ContentCachingResponseWrapper response,
		...
		try {
			...
			if (request.getRequestURI().endsWith(URL.CUSTOMIZATION_REQUESTS_URL.value())) {
				auditService.manageCustomizationRequestsAuditTrails(request, response);
			}
		} finally {
